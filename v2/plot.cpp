/* ----------hwdong的教学资源---------------
   教学博客：https://hwdong.net
   youtube频道： hwdong
   twitter： hwdong
   B站和微博：hw-dong
   qq群：101132160
   微信公众号：hwdong编程
   C++17从入门到精通（源代码）：https://github.com/hwdong-net/cplusplus17
*/

#include <iostream>

#include "ChGL.h"
#include "plot.h"


#if 0
int main() {
	const int w = 80, h = 30;
	if (!initWindow(w, h)) {
		return 1;
	}

	
#if 0
	int n = 7;
#if 0
	double x[] = { 1,40,79,118,157,160,190,250 };
	double y[] = { 4.42978e+10 ,3.01868e+10 ,1.4965e+10 ,8.07343e+09 ,8.07343e+09 ,
	 3.44199e+09 ,2.74686e+09 };
#else
	double x[] = { 2014, 1600, 2400, 1416, 3000, 3670, 4500 };//房屋面积
	double y[] = {400, 330, 369, 232, 540, 620, 800};//房屋价格
#endif
	

#else 
	double x[]{	9.16481174938805,  3.9617176989763605,  1.9118988617843014,
		4.770872353143195,	8.96268633959237,  4.347497877496233,
		0.7837488406009996, 9.003451281535993, 	9.219537986787007,
		0.14895852444561486 };
	double y[]{ 19.3296234987761, 8.923435397952721, 4.823797723568603,
		10.54174470628639,18.92537267918474, 9.694995754992465, 2.567497681201999,
		19.006902563071986,19.439075973574013, 1.2979170488912297 };
	int n = 10;
#endif
	plot(x, y, n, w, h);

	
	return 0;
}
#endif
